...
@Composable
fun EmailBlockerSheet(action: (BlockerAction) -> Unit) {
    val sheetController = LocalSheetCloseController.current
    ZBottomSheetContainer(
        title = stringResource(R.string.email_not_verified),
        illustrationIcon = ZIcons.ic_glass_email.asZIcon,
        ...
        footer = {
            ZPrimaryButton(
                ...
                onClick = {
                    ...
                    action.invoke(BlockerAction.VerifyEmail)
                },
                tagId = "primary_verify_email",
            )
